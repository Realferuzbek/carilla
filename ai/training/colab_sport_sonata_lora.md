# LEGACY: Sport Sonata LoRA Colab Spec (Not Used for Paper 1)

This document is legacy and is not part of Paper 1 (`<carilla_sonata>`, E0-E3) runs.

# Colab spec: Sport Sonata LoRA training (diffusers)
...
- Training: LoRA fine-tuning on ~50–100 Sport Sonata images at 768x768.
...
3) **Set paths**: define `DATA_DIR` pointing to the folder containing `images/` and `captions.txt` (from `processed/sport_sonata`). Define output/checkpoint dirs.  
...
9) **Checkpointing**: save periodic LoRA checkpoints and a final `sport_sonata_lora.safetensors` into Drive.  
10) **Test cell (img2img)**: run an img2img pass on a sample Hyundai Sonata, white, 2020, Asia/Uzbekistan spec, '6-series' photo using the trained LoRA; visualize outputs inline in Colab.  
11) **Cleanup/notes**: remind to unmount Drive if needed.

## After training
- Download the final `sport_sonata_lora.safetensors` file from Drive.  
